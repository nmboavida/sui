    ...
    pub fn insert_generic(&mut self, authority: AuthorityName, s: S) -> InsertResult<()> {
        ...
        if votes > 0 {
            ...
            if self.total_votes >= self.committee.threshold::<STRENGTH>() {
                ...
                InsertResult::NotEnoughVotes { bad_votes: 0 }
            }
        } else {
            InsertResult::Failed {
                            ...
                            Err(_) => {
                                ...
                                for (name, sig) in &self.data.clone() {
                                    if let Err(err) = sig.verify_secure(
                                        ...
                                        // TODO(joyqvq): Currently, the aggregator cannot do much with an authority
                                        // that always returns an invalid signature other than printing a warning and
                                        // record metrics. It is possible to add the authority to a denylist or use
                                        // other ways to punish the byzantine authority.
                                ...
                                InsertResult::NotEnoughVotes { bad_votes }
                            }
                        }
                    }
