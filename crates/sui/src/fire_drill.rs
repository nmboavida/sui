...
pub struct MetadataRotation {
    ...
    // /// CLI command to shut down the node binary
    // #[clap(long = "node-shutdown-command")]
    // node_shutdown_command: Option<String>,
    // /// CLI command to start the node binary
    // #[clap(long = "node-start-command")]
    // node_start_command: Option<String>,
}
...
async fn run_metadata_rotation(metadata_rotation: MetadataRotation) -> anyhow::Result<()> {
    ...
    // info!("Shutted down Sui Node program.");
    ...
    //         .output()
    //         .expect("failed to execute process");
    // info!("Restarted Sui Node program.");
    Ok(())
}
...
async fn update_next_epoch_metadata(
    ...
    let mut backup_config_path = sui_node_config_path.clone();
    ...
    // Network address
    let mut new_network_address = Multiaddr::try_from(
        self_validator
            .net_address
            .clone(),
    )
    .unwrap();
    info!("net: {:?}", new_network_address);
    ...
    let mut new_listen_address = config.p2p_config.listen_address.clone();
    info!("listen: {:?}", new_listen_address);
    ...
    let mut new_primary_addresses = Multiaddr::try_from(
        self_validator
            .primary_address
            .clone(),
    )
    .unwrap();
    info!("primary: {:?}", new_primary_addresses);
    // pop out udp
    ...
    .unwrap();
    info!("worker: {:?}", new_worker_addresses);
    // pop out udp
    ...
    // Save new config
    let mut new_config_path = sui_node_config_path.clone();
    new_config_path.pop();
    ...
    update_metadata_on_chain(
        ...
        vec![CallArg::Pure(
            bcs::to_bytes(&new_network_address.to_vec().clone()).unwrap(),
        )],
        config.sui_address(),
        &sui_client,
    )
    ...
    update_metadata_on_chain(
        ...
        vec![CallArg::Pure(
            bcs::to_bytes(&new_external_address.to_vec().clone()).unwrap(),
        )],
        config.sui_address(),
        &sui_client,
    )
    ...
    update_metadata_on_chain(
        ...
        vec![CallArg::Pure(
            bcs::to_bytes(&new_primary_addresses.to_vec().clone()).unwrap(),
        )],
        config.sui_address(),
        &sui_client,
    )
    ...
    update_metadata_on_chain(
        ...
        vec![CallArg::Pure(
            bcs::to_bytes(&new_worker_addresses.to_vec().clone()).unwrap(),
        )],
        config.sui_address(),
        &sui_client,
    )
